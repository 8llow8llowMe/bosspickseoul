...
# `python` 이 exit 49 로 죽는다). Git Bash 만 있으면 어느 PC 에서든 돈다.
#
# 검사 항목:
...
#   6. .gitignore 에 걸려 추적되지 않는 파일이 없는지 — `.claude/*` 를 통째로 무시하므로
#      예외를 빠뜨리면 정의가 로컬에만 남는다
#
# 실행:
...
for file in "$agents_dir"/*.md; do
  ...
  # 7. gitignore — 무시되면 이 PC 에만 남는다
  if git -C "$repo_root" check-ignore -q "$rel"; then
    fail "Ignored by .gitignore (add an exception so the definition is tracked): $rel"
  fi
done
