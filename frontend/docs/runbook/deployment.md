...
> 라벨은 **PR 을 만들 때** 붙인다. 머지 직전에 기억해서 붙이는 것에 기대면 빠뜨린다 —
> 실제로 #138·#140·#141·#142 가 라벨 없이 머지됐다. PR 생성 명령의 정본은
> `frontend/CLAUDE.md` 의 "PR 생성 규약" 이다.

**Dependabot PR 은 구조적으로 라벨 없이 온다.** `.github/dependabot.yml` 은 `dependencies`
만 붙인다 — 봇이 배포 라벨을 붙이면 매주 자동으로 dev 배포가 나가므로 일부러 그렇게 뒀다.
그래서 봇 PR 은 **머지 전에 사람이 배포 라벨을 붙일지 정해야 한다.** `frontend/` 런타임
의존성이 바뀌었으면 `frontend-web` 을 붙여 그 PR 이 직접 배포를 끌고 가게 한다. 안 붙이면
배포도 CI 도 없이 develop 에 쌓이고, 다음 무관한 FE PR 이 머지되는 순간 함께 나가 dev 가
깨져도 원인 PR 을 특정할 수 없다.

> 실제로 2026-09-11 에 PR #327(next·react·firebase·axios 갱신)이 라벨 없이 올라왔고,
> 같은 날 Dependabot 이 BE PR 13건(#331~#343)을 한 번에 열었다. 봇 PR 트리아지는
> `dependabot.yml` 을 열어보지 않는 사람이 하므로 이 규칙을 여기 적어 둔다.

라벨을 빠뜨리고 머지했다면 잡을 수동 실행하면서 **`FORCE_DEPLOY` 를 체크**한다. 변경 감지와 라벨 게이트를 함께 우회한다. (같은 커밋 재빌드는 변경 파일이 0건이라 라벨만 우회해서는 여전히 생략된다)

**배포하지 않는 브랜치 빌드는 CI 도 돌지 않는다.** 라벨 없는 머지나 develop 직접 push 로 트리거된 빌드는 "배포 대상 아님 - 생략" 으로 끝난다. CI 게이트는 PR 빌드가 담당하므로 여기서 반복할 이유가 없고, backend/ 만 바뀐 push 가 프론트 잡을 FAILURE 로 만드는 것을 막는다. 단 `SKIP_DEPLOY` 수동 실행은 "배포 없이 빌드만 확인" 이 목적이므로 CI 를 그대로 돈다.
