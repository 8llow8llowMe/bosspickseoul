...
| simulation | [simulation](./simulation/simulation.md) | `(shell)/simulation,compare,report`              | 🟡 계약 정렬 + 입력 화면 구현·리포트 대기           | 백엔드 `/api/v1/simulations/**` 신설(2026-08) 반영. 타입·API 클라이언트·지원 업종 30종 상수를 V2로 전면 교체하고 V1 잔재는 `*-v1-legacy` 모듈로 격리(삭제 예정). 세부: [simulation-report](./simulation/simulation-report.md). 리포트는 폴링·SSE 없는 **단일 POST 동기 계산**, 금액 단위 **만원**·면적 **㎡**. `genderAgeAnalysis`/`seasonAnalysis` null은 오류가 아니라 섹션 숨김. 성별·연령 매출은 **자치구×업종 전체** 집계라 범위 라벨 필수(점포 예상 매출로 오독 위험). `periodCode`는 UI 비노출. 공유·이력 삭제는 백엔드 미지원으로 범위 제외. 6개 라우트는 UI 슬라이스까지 `SimulationUnavailablePage` 유지                                                                                                                                                                                                                                                                                                                                                        |
