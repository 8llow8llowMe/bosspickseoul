...
| recommend  | [recommend](./recommend/recommend.md)    | `(shell)/recommend`                              | 🟡 명세 작성중(조건 선택 UX)                        | 공통명세 미작성. 조건 선택 UX 재설계 세부명세 작성 완료(구현 대기) — [condition-selector](./recommend/condition-selector.md). 드롭다운 3개 → **조건 바 한 줄 + 패널 내 선택 뷰 + 지도 뱃지 지름길**. 핵심 원칙은 「목록이 정본, 지도는 가속기」 — 지도 영역 질의가 뷰포트 기반이라 첫 진입에서 자치구 25개 중 21개만 뱃지가 뜬다(강동·관악·금천·도봉 누락, 실측). 그래서 지도는 완전 목록을 대체할 수 없다. 같은 `OptionPicker` 부품을 `/analysis` 선택 패널에도 끼워 검색을 준다(레이아웃은 유지 — 분석은 검증 과업이라 조건이 늘 보여야 하고 좌측 고정 패널이라 지도를 안 가린다). `/recommend` URL 상태 도입과 분석↔추천 딥링크는 D8 로 분리.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
