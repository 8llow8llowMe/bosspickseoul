...
담기지만, **복원되는 것은 조건까지다.** 결과는 `useMutation`이 들고 있어 새로고침·링크 공유
시에는 `비교하기`를 한 번 더 눌러야 나온다. 링크를 받은 사람은 무엇을 비교하려던 링크인지
결과 없이는 알 수 없다.

해소 방향은 리포트 화면과 같은 모양 — 조건의 정본이 쿼리스트링이므로 `useQuery` +
`enabled: request !== null`로 옮긴다. 조건 **편집**의 정본은 여전히 편집기 상태이며, URL은
`비교하기`를 누를 때만 갱신한다(키 입력마다 재계산되면 안 된다).
