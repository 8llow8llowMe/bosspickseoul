...
### TC-005. 준비 상태 화면 유지 (기존)

| 항목      | 내용                                                           |
| --------- | -------------------------------------------------------------- |
| 검증 대상 | S3 #2~#5 미구현 구간                                           |
| 사전 조건 | 6개 simulation 라우트                                          |
| 실행      | route source가 `SimulationUnavailablePage`만 마운트하는지 확인 |
| 기대 결과 | UI 슬라이스 전까지 실패 예정 요청이 발생하지 않는다.           |
| 우선순위  | P1                                                             |
| 비고      | `app/(shell)/simulation/simulation-routes.test.ts` (기존 유지) |
...
| TC-005 | S    | 정상 | 준비 상태 화면 유지 | P1       | P         | UI 슬라이스 전까지           |
...
1. **입력 마법사 UI** (S3 #2) — `SimulationUnavailablePage`는 이 슬라이스가 끝날 때까지 유지한다.
2. **리포트/비교 화면** (S3 #3, #4)
3. **저장·이력** (S3 #5)
