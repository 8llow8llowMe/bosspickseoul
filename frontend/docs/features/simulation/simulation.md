...
| 2   | 입력 마법사           | 프랜차이즈 여부 → 자치구 → 업종 → 매장 크기·층을 **한 번에 한 단계씩** 받아 리포트 요청을 만든다.              | [세부 명세](./simulation-report.md) D4-1 · **D4-1-1** | 부분 구현 |
...
> **#2 가 「부분 구현」인 이유** — 순서(D4-1)와 게이팅은 구현돼 있지만 네 단계가 **한 화면에
> 전부 펼쳐져 있어** 1440x900 에서 버튼 59개가 동시에 놓인다. D4-1-1 이 노출 방식을
> 「한 번에 하나」로 확정했고, 그 전환이 남은 작업이다.
>
