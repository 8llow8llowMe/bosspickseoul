...
| 2   | 입력 마법사           | 프랜차이즈 여부 → 자치구 → 업종 → 매장 크기·층을 **한 번에 한 단계씩** 받아 리포트 요청을 만든다.              | [세부 명세](./simulation-report.md) D4-1 · **D4-1-1** | 구현 완료 |
| 3   | 리포트 화면           | 총비용·비용 구성·권리금·유사 프랜차이즈·성별연령·성수기를 렌더한다. 결측 섹션은 숨긴다.                        | [세부 명세](./simulation-report.md) D4-3              | 구현 완료 |
| 4   | 비교 화면             | 조건이 다른 두 리포트를 병렬 계산해 나란히 비교한다. 한쪽이 실패하면 비교가 성립하지 않으므로 함께 실패시킨다. | [세부 명세](./simulation-report.md) D5                | 구현 완료 |
| 5   | 결과 저장 / 이력 목록 | 로그인 사용자가 결과를 저장하고 최신순 목록을 페이지네이션으로 본다.                                           | [세부 명세](./simulation-report.md) D4-4              | 구현 완료 |

> **#2 는 2026-09-08 에 「한 번에 한 단계」로 전환됐다**(D4-1-1). 1440×900 실측: 한 화면
> 버튼 59개 → 최대 35개(선택지가 30개인 업종 목록이 펼쳐진 순간이 최댓값이다), 페이지 높이
> 1301px → 최대 1030px, 펼쳐진 단계 4개 → 1개. 실측 표와 시나리오 결과는
> [단계 전환 실측](./step-flow-verification.md).
>
> #3~#5 의 상태는 2026-09-08 에 실물 대조로 바로잡았다(라우트·컴포넌트 존재 확인).
> 이 표가 오래 「미구현」으로 남아 있어 명세를 읽는 사람이 화면이 없다고 오해할 수 있었다.
...
| 2.2  | 2026-09-08 | S3 #2 를 「한 번에 한 단계씩」으로 확정하고 상태를 「부분 구현」으로 정정(세부 D4-1-1). #3~#5 의 낡은 「미구현」 표기를 실물 대조로 바로잡음           | Claude |
