...
| 연관 세부 기능    | [분석 결과 리포트](./result.md), `/recommend` 지도 구현                                                                                   |
...
| 분석 결과 CTA    | 클릭                  | 결과 URL 생성           | 데스크톱은 라우트 모달, 모바일은 전체 결과 페이지로 이동한다.    |
...
| 8   | 모바일에서는 지도 위 하단 선택 시트를 사용하되 결과 리포트는 시트가 아닌 별도 전체 페이지로 이동한다. | D6        |
...
- 결과 CTA 선택 시 큰 바텀시트를 확장하지 않고 `/analysis/result` 전체 페이지로 이동한다.
