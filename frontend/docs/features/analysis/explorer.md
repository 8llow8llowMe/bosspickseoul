...
| 「지금 많이 본 상권」 항목 | 클릭/Enter            | 상권 역조회 후 선택 채움 | 자치구·행정동·상권 3단계가 한 번에 채워지고 업종 단계로 이동한다(D5 「인기 상권 지름길」). |
...
4단계를 밟는 전제는 「어느 상권인지 이미 아는 사람」이다. 그렇지 않은 사람에게는
`/recommend` 탈출구가 있지만, **「남들이 지금 많이 보는 상권」으로 곧장 가는 길**은 없었다.

| 규칙      | 내용                                                                                                                                                                                                                                                                                                                  |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 노출 조건 | `activeStep === 'district'` 이고 셸이 콜백을 넘겼을 때만. 자치구를 이미 고른 사람에게 다른 자치구의 상권을 들이밀지 않는다                                                                                                                                                                                            |
| 데이터    | `GET /analysis-rankings?areaType=COMMERCIAL&size=3`. **이 API 만 따로 죽는다**(RANKING_001, 503) — 실패·빈 목록이면 블록을 통째로 빼고 패널의 나머지는 그대로 둔다                                                                                                                                                    |
| 이름 폴백 | `areaName` 이 null 이면 **코드를 적는다**. 상권은 수천 개라 자치구처럼 정적 이름표를 둘 수 없다                                                                                                                                                                                                                       |
| 역조회    | `GET /regions/commercials/{commercialCode}/administration` 을 **눌린 항목 하나에만** 부른다. 목록 N개를 미리 조회하면 N+1 이다                                                                                                                                                                                        |
| 선택 반영 | `selectCommercialWithParents` 로 자치구·행정동·상권 3개를 채우고 활성 단계를 업종으로 옮긴다. 업종은 사용자가 고르므로 결과로 자동 진입하지 않는다                                                                                                                                                                    |
| 카메라    | 역조회는 **좌표를 주지 않는다.** `fitTo.code` 는 이미 로드된 영역에서만 찾으므로 다른 자치구로 건너뛰면 아무 일도 일어나지 않는다 → 이미 켜져 있는 상권 목록 쿼리(`/regions/.../commercials`)에서 중심 좌표가 도착하면 `center` 로 옮긴다. 끝내 오지 않으면 지도는 그대로 두고 목표만 버린다(패널은 이미 채워져 있다) |
| 실패      | 역조회 실패·상위 코드 누락이면 이동하지 않고 블록 안에 이유를 적는다. 반쯤 채워진 화면으로 보내지 않는다                                                                                                                                                                                                              |
...
| TC-AN-011 | D    | 순위 응답에 상권 3건      | 1단계에서 「지금 많이 본 상권」 1위 클릭   | 자치구·행정동·상권이 URL에 채워지고 업종 단계가 열리며 지도가 그 상권으로 이동한다. |
| TC-AN-012 | D    | 순위 API 503 또는 빈 배열 | 1단계 진입                                 | 지름길 블록이 사라지고 자치구 선택은 정상 동작한다.                                 |
...
| 1.2  | 2026-09-06 | 1단계에 「지금 많이 본 상권」 지름길 추가(상권 역조회로 3단계 채움)                                        | Claude |
