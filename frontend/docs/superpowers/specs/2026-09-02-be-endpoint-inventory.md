...
| ~~B3~~ | ~~자치구 상세 지표 5종~~ **오류 — 이미 연동돼 있다** | `GET /districts/{code}/change-indicators`, `/foot-traffic`, `/sales/top-administrations`, `/sales/top-services`, `/stores/top-services` | **이 항목은 틀렸다(2026-09-03 정정).** `/status` 는 `top-ten` **과** `/districts/{code}`(통합 상세) **두 개**를 쓰고, 통합 응답이 이 5종의 스키마를 그대로 담는다(`changeIndicator`/`footTraffic`/`store`/`sales`). 5종은 새 데이터가 아니라 **분할 조회 변형**이다. 실제로 남은 빈틈은 「받아 놓고 안 쓰는 필드 6개」이며 [district-detail-links](../../features/status/district-detail-links.md) 로 재정의했다 |
...
| 날짜       | 항목   | 내용                                                                                                                                                                                                                                                                                |
| ---------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-03 | **B3** | 「미연동 5종」이 아니었다. `/status` 가 이미 통합 상세(`GET /districts/{code}`)로 같은 데이터를 받아 4개 그룹을 전부 그린다. 「`/status` 는 `/districts/top-ten` 하나만 쓴다」도 사실이 아니다. → [district-detail-links](../../features/status/district-detail-links.md) 로 재정의 |
| 2026-09-03 | **B4** | 「우선순위 낮음」이 아니다. `GET /regions/commercials/{code}/administration` 은 **인기 순위의 `COMMERCIAL` 을 화면에 붙이려면 반드시 필요한** 역조회다(이슈 #200). 그 사실을 모르고 「BE 에 역조회가 없다」고 단정했다가 PR #202 로 정정했다                                        |
| 2026-09-06 | **B4** | **완료.** `GET /regions/commercials/{code}/administration` 을 `/analysis` 1단계 「지금 많이 본 상권」 지름길이 쓴다 — 눌린 항목만 역조회해 자치구·행정동·상권 3단계를 채운다([explorer.md](../../features/analysis/explorer.md) D5). `GET /regions/code-lookup` 은 여전히 미연동    |

> **교훈**: 이 인벤토리는 **엔드포인트 단위**로 「연동/미연동」을 셌다. 그런데 BE 가 통합
> 엔드포인트와 분할 엔드포인트를 **둘 다** 제공하면, 통합 쪽을 쓰는 화면이 분할 쪽을
> 「미연동」으로 만든다. **다음에 셀 때는 엔드포인트가 아니라 `dataBody` 스키마로 센다.**
