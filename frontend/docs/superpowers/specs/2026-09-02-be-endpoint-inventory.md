...
| 2026-09-06 | **A 묶음 누락** | **닉네임 수정(`PATCH /members/me`)이 목록에 없었다.** 프로필 화면이 「닉네임 변경은 아직 준비 중이에요」를 띄우는데 BE 엔드포인트는 살아 있다 — A1·A2 와 **정확히 같은 종류**(사용자에게 「준비 중」으로 보이는데 BE 는 끝난 것)인데 집계에서 빠졌다. 원인도 같다: `/members/me` 를 **GET 이 이미 쓰고 있어** 경로 검색이 「연동됨」으로 셌다. **메서드까지 봐야 한다** — 다음에 셀 때는 `(메서드, 경로)` 쌍으로 센다 |
| 2026-09-06 | **A6**          | **완료.** 닉네임 수정(`PATCH /members/me`)을 프로필 회원정보 화면에 붙였다. 응답이 수정된 회원 정보 전체라 재조회 없이 캐시에 심어 헤더·사이드바까지 함께 갱신한다. 검증은 `MEMBER_108`(필수)·`MEMBER_109`(10자)이고 **닉네임 중복 규칙은 BE 에 없다**(가입·수정 모두 검사하지 않으며 unique 제약은 이메일뿐) — 인계 문서가 「미확인」으로 남겼던 항목을 실측으로 닫는다. 이로써 **A 묶음 미연동 0건**                |
