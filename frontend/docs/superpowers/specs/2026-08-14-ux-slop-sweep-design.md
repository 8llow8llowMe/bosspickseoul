...
- ⚠️ **PR3·PR4 모두 `/analysis/result` 시각 검증을 못 했다** — 환경 제약(§10 검증). PR-Home 착수 시 함께 확인한다.
...
- **브라우저 시각 검증을 하지 못했다.** dev 서버 런처가 접근 불가한 cwd 로 프로세스를 띄워
  `getcwd: Operation not permitted` → pnpm `EPERM: uv_cwd` 로 죽는다. **기존 `develop-5173`
  설정에서도 동일하게 재현**되므로 이 변경과 무관한 환경 제약이다. 따라서 §5 가 PR3 에서
  넘긴 `/analysis/result` 시각 확인(scrim·sticky 헤더·nav 활성·히어로 제목) **4건도 여전히
  미완**이다. 다음 슬라이스로 그대로 넘긴다.
