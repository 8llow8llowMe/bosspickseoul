...
- **레이아웃**: **단일 화면 2단.** 마법사(단계별 폼 + 상단 4칸 진행 indicator)를 쓰지 않는다.
  - 컨테이너 폭은 `min(1320px, calc(100% - 40px))` — 분석 결과 화면과 같은 값이다. 760px 카드로 가두면 1920px에서 화면의 60%가 빈다.
  - ≥1024px: `minmax(0,1fr) / 380px` 2단. 왼쪽은 조건 4섹션 세로 스택, 오른쪽은 결과 패널 `position: sticky; top: 96px`.
  - ≤1023px: 1단 스택 + **하단 고정 요약 바**(계산 전 = 남은 조건 문구 + `계산하기`, 계산 후 = 총비용 + `자세히`). 본문 하단 여백 96px로 바에 가리지 않게 한다.
  - **왜 마법사를 걷어냈는가**: 조건 사이 의존성은 **업종 → 브랜드 하나뿐**이다. 나머지 셋은 순서가 무관해 단계로 쪼갤 값이 없고, 대신 "뒤 단계에서 앞 단계로 되돌아가기" 비용만 남았다. 그 하나뿐인 순서는 **업종을 고르기 전 브랜드 검색을 비활성화**해서 지킨다(`franchisees`가 `serviceCode` 필수).
- **헤더**: h1(22/750) + 한 줄 설명을 **같은 줄**에 둔다. eyebrow를 두지 않는다 — 매 방문 같은 3층 문구가 상단을 다 먹을 이유가 없다.
- **조건 섹션**(카드 4장): ① 창업 형태 ② 자치구(25) ③ 업종(지원 30종, 브랜드 검색 포함) ④ 매장 조건(크기·층).
...
- **결과 패널**(오른쪽 sticky / 모바일 하단):
  - **계산 전에도 비워 두지 않는다.** 무엇을 계산하는지 한 줄 + 조건 4개의 현재 값 체크리스트(비면 `선택 전`) + `계산하기`(full width) + 남은 조건 안내.
  - 계산 후: 총비용(30/750, tabular-nums) + 프랜차이즈 여부 badge + **조건 요약 행 흐름**(라벨 왼쪽·값 오른쪽). 2열 그리드를 쓰지 않는다 — 항목이 5개면 마지막 하나가 홀로 남아 붕 뜬다.
  - 오류는 계산 CTA **자리를 대체**한다(둘을 동시에 보여주지 않는다).
- **컴포넌트**:
  - 선택지는 전부 **칩 격자**(최소 44px, 선택 시 primary 테두리·`primary100` 배경·우상단 체크). 상권분석 선택 패널과 같은 관용구.
    최소 열 폭은 창업 형태 200 / 자치구 **96** / 업종 132 / 프리셋·층 120. 자치구 96은 375px에서 3열을 유지하는 상한이다.
  - 브랜드 검색: TextField(돋보기 leftSlot, `emphasized`) + 결과 격자(`minmax(240px,1fr)` 흐름) + `더 보기` secondary 버튼. 커서 페이징 10건씩 누적, 마지막 페이지면 버튼 숨김.
    **업종 미선택 상태에서는 감추지 않고 비활성으로 보여준다** — 자물쇠 아이콘 + `업종을 먼저 선택해 주세요` disabled 필드. 단계가 없으니 순서를 이 자리에서 드러내야 한다.
  - 매장 크기: 프리셋 3칩(`소형`/`중형`/`대형`, **㎡·평 병기**) + `면적 직접 입력` TextField(tabular-nums, `emphasized`, 칸 안 `㎡` rightSlot, 평 환산 helperText).
    크기·층 컨트롤은 **max-width로 폭을 제한한다**(크기 520 / 입력칸 220 / 층 340). 세 자리 숫자 받는 칸이 1000px을 가로지르면 무엇을 넣는 칸인지 흐려진다.
...
- **금액 단위는 전부 만원**이다. 표기는 `N억 M만원`(`formatLargeWon`). 원 단위 포매터를 쓰면 1만 배 틀린다.
  ...
  `formatLargeWon` 은 축약이 아니라 자릿수 구분이다(`2,733,782` → `273억 3,782만원`). 억 자리가 딱 떨어지면 `1억`(`1억 0만원` 아님), `0` 은 `0원`.
- **레이아웃**:
