...
export type UseRecommendPreviewOptions = {
  /**
   * 이 연쇄를 시작해도 되는가. 기본 true — `RecommendPreview`(03 패널)는 이미
   * 사용자가 그 스텝에 도달했을 때만 마운트되므로(스티키 모드) 늘 true면 된다.
   *
   * `false`면 세 쿼리 전부 비활성으로 두고 `isLoading`을 강제로 true(=`—`)로
   * 유지한다 — "아직 모른다"를 정직하게 말하는 것이지, 종결된 폴백이 아니다.
   * 카운터(`ProductStory`)가 스토리 섹션이 뷰포트에 들어오기 **전**에도 이 훅을
   * 불러야 해서 생겼다 — 화면 밖에 있는 03 노드 값을 위해 랜딩 첫 페인트부터
   * 행정동·상권·추천 GET 3개를 미리 쏘지 않기 위해서다.
   */
  enabled?: boolean
}
...
export function useRecommendPreview(
  ...
  { enabled = true }: UseRecommendPreviewOptions = {},
  ...
  const administrationsEnabled = enabled && districtCode !== null
  ...
  const commercialsEnabled =
    enabled && districtCode !== null && administrationCode !== null
  ...
  const previewEnabled =
    enabled && serviceCode !== null && commercialCodes.length > 0
   ...
   *
   * 최상위 `enabled`가 false면(스토리 섹션에 아직 안 닿음) 무조건 로딩이다 —
   * 이건 "더 기다릴 게 없는 종결"이 아니라 "아직 물어보지도 않았다"이므로
   * 폴백(예시)으로 떨어지면 안 된다. 정직하게 "아직 모른다"만 말한다.
   */
  const isLoading =
    !enabled ||
