...
public class AiReportProcessor {
    ...
    private CommercialAiSourceData buildCommercialSourceData(
        ...
        return CommercialAiSourceData.builder()
            ...
            .peakFootTrafficDayOfWeek(PromptFormatterSupport.formatTopEntry(PromptFormatterSupport.orderedMap("월", footTraffic.byDayOfWeek().mondayFootTraffic(), "화", footTraffic.byDayOfWeek().tuesdayFootTraffic(), "수", footTraffic.byDayOfWeek().wednesdayFootTraffic(), "목", footTraffic.byDayOfWeek().thursdayFootTraffic(), "금", footTraffic.byDayOfWeek().fridayFootTraffic(), "토", footTraffic.byDayOfWeek().saturdayFootTraffic(), "일", footTraffic.byDayOfWeek().sundayFootTraffic())))
            .peakFootTrafficAgeGroup(PromptFormatterSupport.formatTopEntry(PromptFormatterSupport.orderedMap("10대", footTraffic.byAgeGroup().age10FootTraffic(), "20대", footTraffic.byAgeGroup().age20FootTraffic(), "30대", footTraffic.byAgeGroup().age30FootTraffic(), "40대", footTraffic.byAgeGroup().age40FootTraffic(), "50대", footTraffic.byAgeGroup().age50FootTraffic(), "60대 이상", footTraffic.byAgeGroup().age60PlusFootTraffic())))
            ...
            .peakSalesDayOfWeek(PromptFormatterSupport.formatTopEntry(PromptFormatterSupport.orderedMap("월", sales.amountByDayOfWeek().mondaySalesAmount(), "화", sales.amountByDayOfWeek().tuesdaySalesAmount(), "수", sales.amountByDayOfWeek().wednesdaySalesAmount(), "목", sales.amountByDayOfWeek().thursdaySalesAmount(), "금", sales.amountByDayOfWeek().fridaySalesAmount(), "토", sales.amountByDayOfWeek().saturdaySalesAmount(), "일", sales.amountByDayOfWeek().sundaySalesAmount())))
            .peakSalesAgeGroup(PromptFormatterSupport.formatTopEntry(PromptFormatterSupport.orderedMap("10대", sales.amountByAge().age10SalesAmount(), "20대", sales.amountByAge().age20SalesAmount(), "30대", sales.amountByAge().age30SalesAmount(), "40대", sales.amountByAge().age40SalesAmount(), "50대", sales.amountByAge().age50SalesAmount(), "60대 이상", sales.amountByAge().age60PlusSalesAmount())))
            .largestAgeGenderShare(PromptFormatterSupport.formatTopPercentEntry(PromptFormatterSupport.orderedPercentMap("남성 10대", salesPercent.maleAge10Percent(), "여성 10대", salesPercent.femaleAge10Percent(), "남성 20대", salesPercent.maleAge20Percent(), "여성 20대", salesPercent.femaleAge20Percent(), "남성 30대", salesPercent.maleAge30Percent(), "여성 30대", salesPercent.femaleAge30Percent(), "남성 40대", salesPercent.maleAge40Percent(), "여성 40대", salesPercent.femaleAge40Percent(), "남성 50대", salesPercent.maleAge50Percent(), "여성 50대", salesPercent.femaleAge50Percent(), "남성 60대 이상", salesPercent.maleAge60PlusPercent(), "여성 60대 이상", salesPercent.femaleAge60PlusPercent())))
            ...
            .largestResidentAgeGroup(PromptFormatterSupport.formatTopEntry(PromptFormatterSupport.orderedMap("10대", population.byAge().age10ResidentPopulation(), "20대", population.byAge().age20ResidentPopulation(), "30대", population.byAge().age30ResidentPopulation(), "40대", population.byAge().age40ResidentPopulation(), "50대", population.byAge().age50ResidentPopulation(), "60대 이상", population.byAge().age60PlusResidentPopulation())))
            .averageMonthlyIncomeAmount(income.averageIncome().monthlyAverageIncomeAmount())
            .largestExpenseCategory(PromptFormatterSupport.formatTopEntry(PromptFormatterSupport.orderedMap("식료품", income.expenseByCategory().groceryExpenseAmount(), "의류", income.expenseByCategory().clothingExpenseAmount(), "의료", income.expenseByCategory().medicalExpenseAmount(), "생활용품", income.expenseByCategory().householdExpenseAmount(), "교통", income.expenseByCategory().transportationExpenseAmount(), "여가", income.expenseByCategory().leisureExpenseAmount(), "문화", income.expenseByCategory().cultureExpenseAmount(), "교육", income.expenseByCategory().educationExpenseAmount(), "유흥", income.expenseByCategory().entertainmentExpenseAmount())))
    ...
    private DistrictAiSourceData buildDistrictSourceData(String districtCode, String periodCode, DistrictDetailQueryResult districtDetail) {
        return DistrictAiSourceData.builder()
            ...
            .topStoreServiceSummaries(districtDetail.store().topStoreServices().stream().map(item -> "%s (점포 %s개)".formatted(item.serviceName(), PromptFormatterSupport.formatNumber(item.totalStoreCount()))).toList())
            .topOpenedAdministrationSummaries(districtDetail.store().topOpenedAdministrations().stream().map(item -> "%s (개업 %s개, 개업률 %s)".formatted(item.administrationName(), PromptFormatterSupport.formatNumber(item.openedStoreCount()), PromptFormatterSupport.formatPercent(item.openingRate()))).toList())
            .topClosedAdministrationSummaries(districtDetail.store().topClosedAdministrations().stream().map(item -> "%s (폐업 %s개, 폐업률 %s)".formatted(item.administrationName(), PromptFormatterSupport.formatNumber(item.closedStoreCount()), PromptFormatterSupport.formatPercent(item.closureRate()))).toList())
            .topSalesServiceSummaries(districtDetail.sales().topSalesServices().stream().map(item -> "%s (증감률 %s)".formatted(item.serviceName(), PromptFormatterSupport.formatPercent(item.salesChangeRate()))).toList())
            .topSalesAdministrationSummaries(districtDetail.sales().topSalesAdministrations().stream().map(item -> "%s (매출 %s, 증감률 %s)".formatted(item.administrationName(), PromptFormatterSupport.formatNumber(item.totalSalesAmount()), PromptFormatterSupport.formatPercent(item.salesChangeRate()))).toList())
            .build();
    }
    ...
    private String formatPeerStoreSummary(CommercialPeerStoreQueryResult peerStore) {
        return "%s (점포 %s개, 개업률 %s, 폐업률 %s)".formatted(peerStore.serviceName(), PromptFormatterSupport.formatNumber(peerStore.totalStoreCount()), PromptFormatterSupport.formatPercent(peerStore.openingRate()), PromptFormatterSupport.formatPercent(peerStore.closureRate()));
    }

    private String formatAdministrationSalesService(AdministrationSalesServiceTopQueryResult item) {
        return "%s (매출 %s, 증감률 %s)".formatted(item.serviceName(), PromptFormatterSupport.formatNumber(item.monthlySalesAmount()), PromptFormatterSupport.formatPercent(item.salesChangeRate()));
    }

    private String formatAdministrationStoreService(AdministrationStoreServiceTopQueryResult item) {
        return "%s (점포 %s개, 개업률 %s, 폐업률 %s)".formatted(item.serviceName(), PromptFormatterSupport.formatNumber(item.totalStoreCount()), PromptFormatterSupport.formatPercent(item.openingRate()), PromptFormatterSupport.formatPercent(item.closureRate()));
    }
}
