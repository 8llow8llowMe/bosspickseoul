...
public class DistrictServiceSwaggerConfig {

    @Bean
    public OpenAPI districtServiceOpenApi(Components components) {
        return new OpenAPI()
            .components(components)
            .info(new Info()
                .title("BossPickSeoul 지역 및 지도 서비스 관련 API 명세서")
                .description("District Service 전용")
                .version("v1")
            )
