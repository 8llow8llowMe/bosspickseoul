...
- `CONSUMPTION_ADMINISTRATION` 은 총액만 옮기던 것을 **총액 + 세부 10항목**으로 넓혔다(#415 1단계). 상권 소비가 끊겨 행정동 소비가 대체 원천이 되기 때문이다. 게시된 payload 가 원천 컬럼을 이미 다 들고 있어 **재적재 없이 `--job=project` 재이관만으로 채워진다.** 항목 구성은 상권과 다르다(여가·문화 합산, 기타·음식 추가). commercial-service 조회 도메인은 아직 총액만 읽는다.
...
- `CONSUMPTION_COMMERCIAL` 소득 두 컬럼은 2024+ 원천 부재로 NULL, 조회 시 0. `service_type` 은 원천에 없어 이관이 `service_category` 로 해석해 채운다(#355).
