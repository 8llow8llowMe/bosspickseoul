...
- 하드 래핑은 160자를 기준으로 둔다.
- 메서드 생성자 `record` 파라미터가 160자 안에 들어오면 한 줄로 유지한다.
- 160자를 넘기거나 가독성이 불분명해질 때만 줄바꿈한다.

## 2. Primitive / Wrapper 기준

- `long`, `int`, `boolean` 같은 기본 타입은 기본적으로 primitive를 사용한다.
...
- 단순 조회 / 수정 / 삭제 정도는 DTO를 만들지 않고 개별 파라미터를 우선 사용한다.
- 조회 조건이 4개 이상이거나 `filter + sort + cursor + size`가 함께 오면 `*Criteria` 또는 `*Query` 객체로 묶는다.
- out-port 계약은 adapter 구현 세부 사항을 노출하지 않는다.

## 4. Mapper 기준

- Entity <-> Domain 매핑은 MapStruct를 우선 사용한다.
- 외부 API / 외부 서비스 응답은 adapter에서 `QueryResult`로 변환한다.
...
- `record` 기반 request / response / item DTO에서 각 component의 `@Schema`는 component 바로 윗줄에 둔다.
- component 사이에는 한 줄을 비워서 설명 블록이 눈에 잘 들어오게 맞춘다.
- validation annotation이 함께 있으면 `@Schema` 다음 줄에 이어서 배치한다.
- Lombok `@Builder`와 record 선언 사이는 한 줄을 넘기지 않는다.
...
- 삭제 전략은 복구 요구사항에 맞춰 명시적으로 선택한다.

## 10. Internal Client 규칙

- Spring 백엔드에서 서비스 간 조회 / 연동은 기본적으로 `FeignClient`를 사용한다.
- Feign 인터페이스는 전용 패키지 안에서 `*Client` 이름을 사용한다.
  - 예: `CommercialAnalysisClient`, `RegionAnalysisClient`
- `adapter/out/client`의 adapter는 Feign 응답을 바로 사용하지 않고 `QueryResult` 또는 domain/model로 변환한다.
