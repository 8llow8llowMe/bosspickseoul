...
- 이 문서는 `BossPickSeoul/backend` 작업을 멀티 에이전트 방식으로 나눠 진행할 때의 기준 문서다.
...
역할 프롬프트 파일은 `.claude/agents/be-*.md` 다. Claude Code 에서는 프로젝트 subagent 로 자동 발견되어 호출하고, Codex·Cursor 처럼 BE 전용 역할 파일이 없는 호스트에서는 같은 파일을 역할 프롬프트로 읽어 순차 수행한다. 모델·권한 배정의 정본은 [docs/claude-agents.md](../../docs/claude-agents.md), 작업 분류와 라우팅은 `dev-orchestrator` 스킬이다.
...
- `community-service` 신규 구축
- `ai-service` 구조 정비
- `commercial-service` 대형 컨텍스트 분리
