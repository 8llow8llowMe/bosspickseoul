...
> 서울 열린데이터광장이 상권 단위 소득·소비 제공을 중단해(이슈 #413) 월 평균 소득·소득 구간 응답 항목은 제거됐습니다(`averageIncomeItem`, `keyMetrics.monthlyAverageIncomeAmount`).

> **소비 지출 계약 (이슈 #415).** `GET /{commercialCode}/income` 은 고정 9필드 객체(`expenseByCategoryItem`)를 없애고 **항목 배열** `expenseCategories[{key, label, amount}]` + 합계 `totalExpenseAmount` + 출처 `provenance` 로 바뀌었습니다. 배열 순서가 곧 화면 순서이고 항목 키를 프론트가 하드코딩하지 않습니다. 값은 **해상도 사다리**로 고릅니다.
>
> 1. `income_commercial` 에 지출이 있으면 → `provenance.scope.code = COMMERCIAL`, 상권 네이티브 9항목
> 2. 없으면 소속 행정동의 `income_administration` 세부 항목으로 대체 → `ADMINISTRATION_PROXY`, 행정동 10항목(여가·문화 합산 + 기타·음식 추가)
> 3. 둘 다 없으면 → `expenseCategories`·`totalExpenseAmount` 가 `null` 이고 `UNAVAILABLE` 로 중단 사실만 전합니다 (404 가 아니라 200)
>
> `provenance` 는 값이 없을 때도 항상 채워집니다: `scope`(CodeNameDescriptionMetadata), `scopeCode`·`scopeName`(값을 실제로 가져온 영역), `sourceId`·`sourceLabel`·`sourceUrl`, `effectivePeriodCode`, `disclaimer`. `disclaimer` 는 대체·중단일 때만 채워지고 네이티브에서는 `null` 입니다. 「9개 항목 합계가 0 이면 값 없음」 판정(이슈 #413)은 그대로이고 행정동 쪽도 같은 기준(세부 항목합)으로 판정하며, 세부 항목이 `NULL` 인 레거시 행은 대체 대상이 아닙니다.
>
> `GET /{commercialCode}/summaries/income` 의 **상권 leg** 도 같은 사다리를 쓰되 **총액만** 대체하고, 출처는 응답 최상위 `commercialProvenance` 가 알려 줍니다. `code`·`name` 은 대체를 쓰더라도 상권의 것을 유지하며, 상권 소비 행 자체가 없는 대체 구간에서는 채울 근거가 없어 `name` 이 `null` 입니다. 대체 원천이 되는 행정동은 **요청 파라미터가 아니라 서버가 상권 코드로 해석한 것**입니다 — 상권이 속하지 않은 `administrationCode` 를 보내도 무관한 동의 총액이 상권 값으로 들어가지 않습니다. 자치구·행정동 leg 는 원천이 살아 있어 건드리지 않으며, 행이 없는 지역 단위만 `null` 입니다. 행정동 leg 의 `totalExpenseAmount` 는 세부 항목이 적재된 행이면 항목합이라, 대체 구간에서 행정동 leg 와 상권 leg 는 항상 같은 값입니다.
>
