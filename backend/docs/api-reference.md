...
>
> `GET /{commercialCode}/summaries/income` 의 **상권 leg** 도 같은 사다리를 쓰되 **총액만** 대체하고, 출처는 응답 최상위 `commercialProvenance` 가 알려 줍니다. `code`·`name` 은 대체를 쓰더라도 상권의 것을 유지합니다. 자치구·행정동 leg 는 원천이 살아 있어 건드리지 않으며, 행이 없는 지역 단위만 `null` 입니다.
>
> 대체값은 **점수에 넣지 않습니다.** 같은 행정동에 속한 상권이 전부 같은 값을 받아 상권 간 변별력이 없으므로, 히트맵 기회도·비교 승패 판정·후보 추천은 네이티브 전용 경로를 그대로 씁니다.
