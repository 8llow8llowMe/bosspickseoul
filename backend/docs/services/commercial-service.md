...
- 종합 추천은 총 매출, 개업률, 폐업률(낮을수록 우위), 총 거주인구, 총 시설 수의 승패를 사용한다.
  ...
  `spendingMetrics` 자리는 유지하되 **행정동 대체값(이슈 #415)이 붙어도 되돌리지 않는다** — 대체값은 소속
  행정동 값이라 같은 행정동 안의 두 상권은 항상 동점이고, 다른 행정동끼리면 상권이 아니라 행정동을 비교한
  결과가 승패로 나간다. 비교가 읽는 소비 경로도 네이티브 전용
  (`CommercialQueryProcessor.getIncomeByPeriodCodeAndCommercialCode`) 그대로 둔다.
...
- 상권 -> 행정동 해석은 새 포트를 만들지 않고 프로필이 이미 쓰는 `CommercialRegionQueryPort` 를 재사용한다.
  네이티브가 있으면 아예 부르지 않는다. `/summaries/income` 은 행정동 코드를 요청으로 받고 행정동 leg 를
  채우며 읽은 행을 사다리가 재사용하므로 추가 조회가 없다.
- `/income` 파사드에는 트랜잭션을 걸지 않는다. 사다리 2단계에서 지역 서비스를 Feign 으로 부르므로 DB
  커넥션을 쥔 채 원격 응답을 기다리게 된다.
