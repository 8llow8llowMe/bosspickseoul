...
- 종합 추천은 총 매출, 개업률, 폐업률(낮을수록 우위), 총 거주인구, 총 시설 수의 승패를 사용한다.
  소비(총 지출액)는 원천이 20241 분기부터 전 행 0 을 주는 동안 항상 무승부라 판정에서 뺐다(이슈 #413).
  **행정동 대체값(이슈 #415)이 붙어도 되돌리지 않는다** — 대체값은 소속 행정동 값이라 같은 행정동 안의 두
  상권은 항상 동점이고, 다른 행정동끼리면 상권이 아니라 행정동을 비교한 결과가 승패로 나간다. 비교가 읽는
  소비 경로도 네이티브 전용(`CommercialQueryProcessor.getIncomeByPeriodCodeAndCommercialCode`) 그대로 둔다.
- 대신 `spendingMetrics` 는 **한쪽이라도 네이티브 소비가 없으면 빈 배열**로 내린다(이슈 #415). 지표 값이
  primitive 라 결측을 0 으로밖에 담지 못하는데, 원소가 하나뿐인 묶음은 화면이 통째로 버리지 못해
  「총 지출액 0원 vs 0원」 이 그려진다. 같은 상권·분기를 분석 화면이 「행정동 기준(대체) ○○원」 이라고
  말하는 동안 비교 화면만 0원이라고 말하게 되므로, 값을 지어내는 대신 묶음을 비운다.

## 소비 해상도 사다리 (이슈 #415)

- 상권 단위 소비 원천이 20241 분기부터 전 행 0 이라, 화면에 값을 보여 주는 두 경로만 대체 출처를 쓴다.
  판정 정본은 `CommercialExpenseProvenanceProcessor` 하나다.

  | 단계 | 조건 | `provenance.scope.code` |
  |------|------|--------------------------|
  | 1 | `income_commercial` 에 지출이 있다 | `COMMERCIAL` (네이티브 9항목) |
  | 2 | 소속 행정동 `income_administration` 에 세부 항목이 있다 | `ADMINISTRATION_PROXY` (행정동 10항목) |
  | 3 | 둘 다 없다 | `UNAVAILABLE` (값 `null`, 중단 사실만) |

- 상권 -> 행정동 해석은 새 포트를 만들지 않고 프로필이 이미 쓰는 `CommercialRegionQueryPort` 를 재사용한다.
  네이티브가 있으면 아예 부르지 않는다.
- **소속 행정동은 언제나 서버가 상권 코드로 해석한다.** `/summaries/income` 은 행정동 코드를 요청으로 받지만
  그 값을 대체 원천으로 믿지 않는다 — 공유 링크·북마크·프론트 상태가 꼬여 상권이 속하지 않은 동 코드가 오면
  무관한 동의 총액이 상권 값으로 들어가고 면책 문장이 틀린 동 이름을 단정하기 때문이다. 해석 결과가 요청과
  같으면 행정동 leg 를 채우며 읽은 행을 그대로 재사용하므로 정상 요청에서는 추가 DB 조회가 없다.
- 행정동 leg 의 총액도 **세부 항목이 적재된 행이면 항목합**이다(`IncomeAdministration.displayTotalExpenseAmount`).
  대체 구간에서 상권 leg 가 같은 행의 항목합을 쓰므로, 행정동 leg 만 합계 컬럼을 쓰면 화면이 "같은 값" 이라고
  설명하는 두 줄에 다른 숫자가 그려진다. 세부가 NULL 인 레거시 행(`20233` 이하)만 합계 컬럼으로 폴백한다.
- 상권 소비 행 자체가 없는 대체 구간에서는 상권 leg 의 `name` 이 `null` 이다. 소비 요약 응답 어디에도 상권명이
  없어 채울 근거가 없다(2024년 이후 1,650곳 중 560곳이 이 경로).
- **지역 서비스 Feign 이 섞이는 유스케이스에는 Facade 트랜잭션을 걸지 않는다.** `/income`,
  `/summaries/income`, `/compare`, `/compare-preview`, `/benchmarks`, `/profile` 여섯 경로다. 트랜잭션을 두면
  상대가 느려지는 구간(서킷 오픈 직전 타임아웃)만큼 DB 커넥션이 잡혀 소비와 무관한 조회 API 까지 함께 죽는다.
...
- 히트맵은 소비를 **조회조차 하지 않는다.** 점수식에서 지출 항이 빠진 뒤 `CommercialHeatmapSource.income` 을
  읽는 코드가 0 건이 됐는데도 요청마다 뷰포트 안 상권 전부에 `in` 절 조회를 돌리고 결과를 버리고 있었다.
  필드·인자·벌크 조회·포트 메서드를 함께 제거했다(벌크 조회 6종 -> 5종). 소비를 점수에 되살리려면
  `CommercialHeatmapSource.hasAllMetrics` 와 `computeOpportunity` 를 함께 고치고, 그 전에 **상권 단위** 지출
  원천이 실제로 되살아나야 한다.
