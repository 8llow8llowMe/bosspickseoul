...
`dataset_fact` / `dataset_active_release` 를 이 서비스가 분기마다 골라 읽는 라우팅 조회 경로(`dataset` 컨텍스트, `Legacy/Dataset*Source`, `app.dataset.*`)는 **제거했다**. 팩트 out-port 구현체는 기존 팩트 테이블만 JPA 로 읽는다.
...
- 후속 작업: 기존 팩트 테이블(상권·자치구·행정동)에 공간 버전 등 필요한 컬럼을 추가한 뒤, 적재된 `dataset_fact` 를 그 테이블로 **이관**한다. 클라이언트가 `20241` 이후 분기를 요청하면 같은 테이블에서 변경된 기준의 값을 읽게 되며, 조회 경로에는 소스 분기 로직이 다시 들어가지 않는다.
