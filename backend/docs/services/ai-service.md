...
- **상권 소비는 출처를 프롬프트에 함께 싣는다.** 상권 단위 원천이 `20241` 분기부터 중단돼 commercial-service 가 그 자리를 소속 행정동 값으로 대체하고 출처(`provenance`)를 응답에 담는다(#415). 금액만 적으면 LLM 이 대체값을 그 상권의 실측으로 읽으므로 `[지출]` 섹션에 소비 출처 줄을 넣고, 원천 서비스가 만든 면책 문장이 있으면 「유의」 줄로 그대로 옮긴다. 항목은 스코프마다 구성이 달라(상권 9개 / 행정동 대체 10개) 키를 고정하지 않고 배열을 순서대로 적으며 라벨도 서버가 준 문구를 쓴다. 결측은 기존 `N/A` 표기를 따르고 0 으로 채우지 않는다. 「지출 비중이 가장 큰 항목」의 동일성은 라벨이 아니라 `key` 로 본다 — 라벨을 키로 삼으면 문구가 같은 항목이 둘일 때 뒤의 것이 앞의 것을 덮어써 최댓값 후보에서 조용히 사라진다. 같은 면책 문장은 `[지출]` 과 `[지역 비교]` 에 **한 번만** 싣는다. 반복된 경고는 LLM 이 리포트 본문에도 두 번 옮겨 적게 만든다.
